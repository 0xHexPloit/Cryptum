...
impl <const V: usize> Kyber<V> {
    ...
    ///
    /// Input:
    ///     A bytes array to be hashed
    /// Output:
    ///     A bytes array of length 32
    fn kdf(&self, data: ByteArray) -> ByteArray {
        shake_256(data.get_bytes(), 32).into()
    }
    ...
    ///
    /// Input:
    ///     A bytes array representing a public_key generating by the keygen function.
    ///     A bytes array filled with random values. It should be of length 32
    ///
    pub fn encrypt(&self, public_key: ByteArray, seed: ByteArray) -> (ByteArray, ByteArray) {
        ...
        let expected_public_key_size = self.get_public_key_size();
...
mod tests {
    ...
    fn test_encrypt() {
        ...
        let (ciphertext, secret_key) = kyber.encrypt(public_key, seed);
